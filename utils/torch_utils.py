...
def select_device(device='', batch_size=0, newline=True):
    # device = None or 'cpu' or 0 or '0' or '0,1,2,3'       YOLO v5信息和pytorch信息
...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, weight_decay=1e-5):
    ...
    for v in model.modules():
        ...
        if hasattr(v, 'attn1'):
            if hasattr(v.attn1, 'logit_scale'):
                g[1].append(v.attn1.logit_scale)
            if hasattr(v.attn1, 'q_bias'):
                g[1].append(v.attn1.q_bias)
            if hasattr(v.attn1, 'v_bias'):
                g[1].append(v.attn1.v_bias)
            if hasattr(v.attn1, 'relative_position_bias_table'):
                g[1].append(v.attn1.relative_position_bias_table)
